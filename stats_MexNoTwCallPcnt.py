...
print('stated', sd)
voz_or_en = 'VOZ'
stats_dir = 'stats/AggMexUsrNoTwCall%s/' % voz_or_en
...
df = pd.DataFrame(list(num_calls.values()), index=list(num_calls.keys()), columns=['all', 'nt1', 'nt2'])
...
ax = df[['nt1pct', 'nt2pct']].plot.hist(alpha=0.5, title='overall no t1=%0.2f%% and no t2=%0.2f%%' % (nt1p, nt2p))
fig = ax.get_figure()
