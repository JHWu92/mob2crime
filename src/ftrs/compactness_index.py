...
def cohesion(ref_circle_radius, pairwise_dist_square_avg):
    if ref_circle_radius is None or pairwise_dist_square_avg is None:
        return None
...
def proximity(ref_circle_radius, d2centroid_avg):
    if ref_circle_radius is None or d2centroid_avg is None:
        return None
...
def moment_inertia(raster_areas, d2centroid):
    if raster_areas is None or d2centroid is None:
        return None
...
def mass_moment_inertia(raster_rper, d2mass_centroid):
    if raster_rper is None or d2mass_centroid is None:
        return None
